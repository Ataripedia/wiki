...
Installing a PAK3-030 would require a socket and a complete relocation of the DC-DC converter using cables. Although no known attempts have been made with the STacy, it is conceivable to use a CPU relocater to have the accelerator elsewhere while retaining the DC-DC converter in its original location. While this is known to work in other machines and most notably in Amigas and TF53X-series accelerators, it is doubtful the PAK3-030 would run fast or stable in an extender.
